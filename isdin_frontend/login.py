...
os.environ["GOOGLE_REDIRECT_URI"]=os.environ["MMM_GOOGLE_REDIRECT_URI"]
...
if login_info:
    ...
    if user_email.lower() not in map(str.lower, permitted_emails.keys()):
        st.write(f"This email \"{user_email}\" is not in the list of permitted emails. Please logout and try again with a correct email.")
        
    else:
        login_role = permitted_emails[user_email]
    ...
    with col1:
        if st.button("Sección Administrador"):
            st.markdown(f"<meta http-equiv='refresh' content='0;URL={admin_google_redirect_uri}' />", unsafe_allow_html=True)
